...
U8 SDI_locked(U8 ch)
{
	...
	for (i = 0; i < 16; i++)
	{
		...
		if ((val >> 6) & 0x01)
			v_lock++; //for normal video lock checking
...
*/
	...
	if (val == 0)
	{
		...
		if ((ReadTW6874(0x044 + (ch << 1)) >> 7) & 1)
			SetSignalTW6874(0x044 + (ch << 1), 0x77, 0); // turn off output if it's on;
#endif
		return RES_NONE;
	}
	else
	{
		...
		if (DiracInput[ch] || (((ReadTW6874(ch << 1) & 0xc7) == 0xc5) && (DiracInput[ch] == 0) && DiracAutoFlag)) // even though DiracInput[ch] is 0, still check if it is Dirac case if it is SD input and  DiracAutoFlag is set
		{
			...
			if (DiracAutoFlag && ((!((ReadTW6874(0x054) >> ch) & 1)) || ((ReadTW6874(ch << 1) & 0x80) == 0)))
			{ //if dirac was disabled/status changed, enable it;
				switch (ch)
				{
				case 0:
		...
		{
			if ((DiracAutoFlag) && ((ReadTW6874(0x054) >> ch) & 1))
			{ //if dirac was enabled, disable it;
				switch (ch)
				{
				case 0:
...
*/
	...
	if ((port >= 0) && (port < 4))
	{
		...
		else
		{
			...
			if (val < 2)
			{ // make sure dirac signal is stable here;
				DiracInput[port] = 1;
				TW6874_Ch(port + 1, 1);
...
void CheckChLock(U8 ch)
{
	...
	{
		//锁定次数小于50%或状态发生变化，增益
		printk("stepONE_2_%d_lock_auto,lockCnt:%d\n", ch, lockCnt);
		lock_auto(ch);
		return;
	}
...
void lock_auto(U8 ch)
{
	...
	for (jj = 2; jj < 9; jj += 2)
	{
		...
		for (ii = 2; ii < 9; ii += 2)
		{
			...
			if ((noCrc == 1) && (bothLock == LOOP_COUNT) && (errCnt > 0) && (errCnt < LOOP_COUNT))
			{ // detect crc when lock is stable;
				for (kk = 0; kk < LOOP_COUNT; kk++)
				{
					if ((val & 0x54) == 0x44)
					{
...
*/
	...
	else if (agc == maxLock)
	{
		...
		else
		{
			...
			if ((agc <= 0xa0) && (agcHist[0] > agcHist[2] + 32))
			{ // possible short cable case;
				agc -= 8;
			}
			else if ((agc >= 0xe0) && (agcHist[4] > agcHist[2] + 32))
			{ // possible long cable case;
				agc += 8;
			}
...
*/
	...
	{
#else
	if ((((ReadTW6874(ch << 1) & 3) == 1) && (DiracAutoFlag == 1)))
	{ // Dirac case;
#endif
...
void ReadANC(U8 port, U8 pos, U8 did, U16 line)
{
	...
	if (line > 0)
	{ //line number specified; //only take one configuration here;
		SetSignalTW6874(0x810 + ((U16)ch << 8), 0x70, line & 0xff);
		SetSignalTW6874(0x811 + ((U16)ch << 8), 0x70, line >> 8);
	}
	SetSignalTW6874(0x821 + ((U16)ch << 8), 0x76, 0x0);
